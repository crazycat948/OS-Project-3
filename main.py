...
from btree import insert, search, print_tree
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python main.py <command> <filename> [args]")
        return
    ...
    if command == "create":
        ...
        if len(sys.argv) != 5:
            print("Usage: python main.py insert <filename> <key> <value>")
            return
        key = int(sys.argv[3])
        ...
        if len(sys.argv) != 4:
            print("Usage: python main.py search <filename> <key>")
            return
        key = int(sys.argv[3])
        search(filename, key)
        ...
        if len(sys.argv) != 3:
            print("Usage: python main.py print <filename>")
            return
        print_tree(filename)
